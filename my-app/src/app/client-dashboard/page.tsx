...
export default async function ClientDashboard({ searchParams }: DashboardProps) {
  ...
  // Fetch products based on page and search query
  const { products, totalPages } = await getProducts(currentPage, query);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-6 border-t border-slate-800">
          {products.length > 0 ? (
            ...
            <div className="col-span-full text-center py-20 text-slate-500">
              {/* FIXED: Using &quot; instead of " to satisfy ESLint */}
        ...
        {totalPages > 1 && (
          <div className="mt-12 flex justify-center items-center gap-2 pt-8 border-t border-slate-800">
            {Array.from({ length: totalPages }, (_, i) => {
              ...
              // Maintain query parameter in pagination links
              const pageUrl = query 
                ? `/client-dashboard?page=${pageNum}&query=${encodeURIComponent(query)}`
                : `/client-dashboard?page=${pageNum}`;
